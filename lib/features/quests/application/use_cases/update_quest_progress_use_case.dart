...
/// Moves a quest's progress for one day by one increment/decrement/binary
/// completion step. This is the write path Phase 8 adds for quantity and
/// duration quests (binary quests keep using [CompleteQuestUseCase] directly
/// via the existing `CompleteQuestButton`/`CompleteQuestController` — see
/// this class's own doc for why `QuestProgressPolicy.nextValue` still
/// accepts `complete` as a binary operation regardless: it keeps the policy
/// itself type-complete even though this use case's only caller currently
/// exercises it for quantity/duration).
///
...
///   always safe because `CompleteQuestUseCase` prices same-day repeats at
///   a diminishing (never duplicated) rate. That's true for *repeatable*
///   quests, but for a quest with `repeatabilityRule == null` — one-time,
///   by the only repeatability signal this domain model actually wires up
///   anywhere — it let a decrement-then-reincrement (same day, or even a
///   later day) earn a second, diminished-but-nonzero `XpTransaction`,
